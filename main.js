...
function addSingleElement(drink) {
  ...
  singleElementContainer.innerHTML = `
        <div class="single-element-container__drink">
            <div class="single-element-container__drink-name">${
    drink.strDrink
    }</div>
            <div class="single-element-container__drink-category">${
    drink.strCategory
    }</div>
            <img src="${
    drink.strDrinkThumb
    }" class="single-element-container__drink-img" />
            <button class="hide-details">Hide details</button>
        </div>
        <div class="single-element-container__ingredients">
            <ul class="single-element-container__element-list">${ingredients
      .map(
        (el) =>
          `<li class="single-element-container__element-list__single-element">${el}</li>`
      )
      .join("")}</ul>
            <p class="single-element-container__instructions">${
...
function addElementToDOM(data) {
  container.innerHTML = data.drinks
    .map(
      (drink) =>
